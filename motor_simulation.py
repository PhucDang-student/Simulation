...
class SimpleMotor:
    def __init__(
        self,
        I_arm=0.01,        # Effective inertia at the joint [kg*m^2]
        I_motor=0.001,
        d=0.1,        # Viscous joint damping [Nm*s/rad]
        g=10.0,        # Gearbox ratio
        Kp=50.0,        # Proportional gain
        Kd=1.0,        # Derivative gain
        tau_max=5.0,   # Maximum motor torque [Nm]
        ...
        self.I_eff =  self.I_arm + self.I_motor * self.g**2
    ...
    def simulate(self, theta_target):
        ...
        for i in range(self.steps):
            ...
            # Apply torque saturation (ideal torque-controlled motor limitation)
            motor_torque = np.clip(motor_torque, -self.tau_max, self.tau_max)
            
            # 2. System Dynamics
            # Torque applied at the joint (amplified by gear ratio)
            joint_torque = self.g * motor_torque
            
            # Equation of motion: J*theta_ddot + b*theta_dot = tau_j
            theta_ddot = (joint_torque - self.d * theta_dot) / self.I_eff
...
def on_slider_change(val):
    ...
    # Update y-axis limits dynamically based on the new data
    max_vel = np.max(np.abs(theta_dot_hist))
    if max_vel > 0:
        ax_vel.set_ylim(-max_vel * 1.2, max_vel * 1.2)
        
    max_acc = np.max(np.abs(theta_ddot_hist))
    if max_acc > 0:
        ax_acc.set_ylim(-max_acc * 1.2, max_acc * 1.2)
